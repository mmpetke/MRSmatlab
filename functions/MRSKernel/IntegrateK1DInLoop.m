...
%                     K_part1 = gamma * earth.erdt^2 * 3.29e-3 * B_comps_Tx.e_zeta .* ...
%                         sin(0.5 * gamma * measure.pm_vec(n) * (B_comps_Tx.alpha - B_comps_Tx.beta));
%                     
...
%                 end
            ...
            case 3 % Mp after imperfect PP-ramp switch-off
                ...
                for n = 1:length(pm_vec)
                    ...
                    K_part1 = gamma * earth.erdt^2 * 3.29e-3 .* m;
                    
                    K_part2 = B_comps_Rx.e_zeta .* (B_comps_Rx.alpha + B_comps_Rx.beta);
                    
                    K(n,:) = sum(sum(K_part1 .* K_part2 .*dh*dz));
    ...
    case 2 %'T1' double pulse T1 kernel
        ...
        for n = 1:length(pm_vec) % loop pulse moments            
            ...
            for td = 1:length(measure.taud) % loop tau
                ...
                K((td-1)*length(pm_vec) + n,:) = sum(sum(K_part1 .* K_part2 .* K_part3 .*dh*dz));
            end
