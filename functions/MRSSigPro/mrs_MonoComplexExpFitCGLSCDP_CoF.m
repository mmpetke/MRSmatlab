function [m,e,J] = mrs_MonoComplexExpFitCGLSCDP_af(y,t,ini,lb,ub)
...
for it=1:20
    ...
    dm         = inv(G'*G + 10/sqrt(it)*eye(6))*G'*dD;
...
function D = ForwardSignal(x,t)
    D = x(1)*exp(-t/x(2)).*complex(cos(2*pi*x(3).*t+x(4)),sin(2*pi*x(3).*t+x(4))) + complex(x(5), x(6));
end
