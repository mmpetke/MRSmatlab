...
%   $Revision: 1.7.4.6 $  $Date: 2010/11/08 02:33:28 $
...
if ncols==6 && L==1 && na<=2,
    if b(4)==1,
        warning(generatemsgid('ParseSOS'), ...
        ['The first two inputs are interpreted as an SOS matrix and G vector. ', ...
        'Use column vectors if you meant to specify a transfer function.']);
    else
        warning(generatemsgid('ParseB'), ...
        ['The first two inputs are interpreted as a transfer function. ', ...
        'Normalize the denominator coefficients (a01 must be equal to 1) if you meant to specify an SOS matrix.']);
    end
end
...
if issos,
    ...
    if ng>L+1,
        error(generatemsgid('InvalidDimensions'), ...
            'There must be at most %d scale values.', L+1);
    elseif ng==L+1,
...
for ii=1:L
    ...
    [dummy,zo] = filter(b(:,ii),a(:,ii), xt, zi(:,ii)*xt(1)); % yc1 not needed
    ...
    [dummy,zo] = filter(b(:,ii),a(:,ii), yc3(end:-1:1), zi(:,ii)*yc3(end));
...
for ii=1:L
    ...
    [dummy,zo] = filter(b(:,ii),a(:,ii),xt,zi(:,ii)*xt(1,:)); % outer product
    [yc2,zo] = filter(b(:,ii),a(:,ii),xc,zo);
    xt = bsxfun(@minus, 2*xc(end,:), xc(end-1:-1:end-nfact,:));
    yc3 = filter(b(:,ii),a(:,ii),xt,zo);
    
    [dummy,zo] = filter(b(:,ii),a(:,ii),yc3(end:-1:1,:),zi(:,ii)*yc3(end,:)); % outer product
    yc5 = filter(b(:,ii),a(:,ii),yc2(end:-1:1,:), zo);
