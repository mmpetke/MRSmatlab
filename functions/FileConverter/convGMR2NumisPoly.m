function convGMR2NumisPoly(gui,fdata)
...
for irec = 1:length(fdata.header.fileID)
    ...
    switch fdata.header.Qsampling
        case 0 % standard GMR, one file contains all pulse moments
            % separate this one file into single pulse moments and write in Poly format
            % data is structured as --> data.record{pulsemoment}
            % sometime GMR pulse moments are reverse ordered or anything else
            [dummy,index]  = sort(data.q1); 
            for n=1:length(data.q1)
                mrs_setguistatus(gui,1,['Converting ' num2str(n) ' out of ' num2str(length(data.q1)) ' qs'])
                % sort during writepoly --> irec is stack, index is to sort q
                sampleFrequency = writePolyData(data,fdata,index(n),irec,n); 
            end
