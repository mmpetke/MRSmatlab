...
function fs = writePolyData_no_decimating(data,fdata,iq,irec,nq)
...
for irx=1:fdata.header.connectedchannels
    if ~fdata.UserData(irx).looptask==0
        ...
        polyout.receiver(irx).SampleFrequency = fdata.header.sampleFreq;
        polyout.receiver(irx).deadtime        = fdata.header.deadtime;
        nsample.sig2                          = length(polyout.receiver(irx).signal(2).V);
        nsample.dead1                         = polyout.receiver(irx).deadtime; % be aware must be integer
    end
end

switch fdata.header.info(1)
    case 4
        ...
        for irx=1:fdata.header.connectedchannels
            if ~fdata.UserData(irx).looptask==0
                ...
                nsample.dead2                         = polyout.receiver(irx).deadtime; % be aware must be integer
                nsample.pause                         = 10; % 10ms between end of fid1 and second pulse
            end
        end
end
...
polyout.transmitter.FreqReelReg9833 = fdata.header.frequency;
...
fwrite(myfile, fdata.header.taup*1e3,'int32');
...
fwrite(myfile, fdata.header.capacitance,'float32');
% TParameter.UGen
fwrite(myfile, -1,'float32');
% TParameter.x6
fwrite(myfile, -1,'int32');
...
for irx = 1:fdata.header.connectedchannels    % for each receiver
